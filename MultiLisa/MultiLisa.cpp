...
int main()
{
	...
	ReleaseSemaphore((HANDLE)array[5], 1, NULL);	// Запускаем поток MultiElement
...
DWORD WINAPI work(LPVOID array) {
	...
	int offset = 6;						//
	int* tmp = new int[arrayL[0]- 6];	// Создаём временный массив
...
DWORD WINAPI MultiElement(LPVOID array) {
	...
	WaitForSingleObject((HANDLE)arrayL[5], INFINITE);	// Ждём сигнал
